...
% Modified 2/17/2023 by Emily Mongold for CEE288 class example
...
%% load hazard curve
% IM values
x = [logspace(log10(0.001), log10(2), 100) 2.1];
...
% Use this to generate the fragility function curve
pFail = ...;
...
xlim([0 1.5])
legend('Fragility, \theta = 0.5 g', 'Ground-motion hazard', 'location', 'southeast')

    
%% discretize down for tabular output
xShort = [0.001 0.01 0.1:0.1:1.5]';
lambdaShort = interp1(...); % discretized hazard curve (interpolate values from coninuous)
dLambdaShort =  ...;  % derivative of hazard curve
pFailShort =  normcdf(...);

failRateDiscrete = sum(pFailShort .* dLambdaShort)
