function [median, sigma, period1] = gmm_ab2006_stable(T, M, Rrup, Vs30, stress)

% Created by Lynne Burks, 1/25/10
%
...
%                 = 140 for typical faults (default value, if not provided)
...
% if stress input is not provided, set it to the default value
if nargin < 5
    stress = 140;
end
...
if Vs30>=2000 || Vs30==Vref
    ...
    pgaBC = gmm_ab2006_stable(0, M, Rrup, Vref, stress);
    if pgaBC<=60
...
logSa = c(1) + c(2).*M + c(3).*M.^2 + (c(4)+c(5).*M).*f1 + ...
    (c(6)+c(7).*M).*f2 + (c(8)+c(9).*M).*f0 + c(10).*Rrup + S;

%**************************************************************************
% COMPUTE STRESS ADJUSTMENT FACTOR USING EQN. (6)
%**************************************************************************
logSF2 = min((delta_val+0.05), (0.05 + delta_val*max(M-M1_val, 0)/(Mh_val-M1_val)));
